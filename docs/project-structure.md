...
| `src/cli/commands/`     | Command implementations for host, tray, domains, diagnostics, and TUI launch |
...
- `setup.ts`: interactive and non-interactive config creation.
- `install.ts`: root install, daemon plist generation, tray install trigger.
...
- `tray.ts`: per-user tray LaunchAgent install and Rosetta probe.
- `tui.ts`: launches the Ink dashboard.

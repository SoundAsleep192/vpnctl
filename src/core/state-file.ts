...
// State older than this means the monitor daemon stopped ticking; the tray then
// shows "unknown" rather than a stale green/red that no longer reflects reality.
...
export type TrayStatus = "protected" | "starting" | "fail-closed" | "unknown";
...
export function parseStateFile(text: string): VpnState | null {
  ...
  const tunnelStarting = "tunnelStarting" in data && typeof data.tunnelStarting === "boolean" ? data.tunnelStarting : false;
  return { tunnelUp: data.tunnelUp, trustedIface, sinkholeActive: data.sinkholeActive, tunnelStarting, timestamp: data.timestamp };
}
...
export function classifyState(state: VpnState | null, nowMs: number, staleMs: number = STATE_STALE_MS): TrayStatus {
  ...
  if (!state.tunnelUp && state.tunnelStarting) return "starting";
...
export async function writeStateFile(
  ...
  const state: VpnState = {
    tunnelUp,
    trustedIface,
    sinkholeActive: !tunnelUp,
    tunnelStarting: !tunnelUp && tunnelStarting,
    timestamp: nowMs,
  };
